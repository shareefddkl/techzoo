...
class User < ActiveRecord::Base
 ...
 validates_acceptance_of :terms_of_service, :message => "must be agree with terms"
  ...
  class << self
    def authenticate(email, password)
      if user = find_by_email(email)
        if user.status == 'active' and user.hashed_password == encrypt_password(password, user.salt)
          user
        end
      end
    end
  ...
  def json_data
    { :name => self.display_name.to_s, :id => self.id, :status => self.online_status, :email => self.email}
  end
